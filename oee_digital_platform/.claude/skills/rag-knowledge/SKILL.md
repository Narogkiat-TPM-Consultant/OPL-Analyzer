...
```bash
uv run oee_digital_platform cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run oee_digital_platform cmd rag-search "your question" --collection docs        # semantic search
uv run oee_digital_platform cmd rag-collections                                     # list collections
uv run oee_digital_platform cmd rag-stats                                           # chunk/vector counts
uv run oee_digital_platform cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run oee_digital_platform cmd rag-sources                  # list
uv run oee_digital_platform cmd rag-source-add               # add (interactive)
uv run oee_digital_platform cmd rag-source-sync --all        # trigger a sync
```
