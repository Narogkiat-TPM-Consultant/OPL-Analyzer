...
OEE Digital Platform scaffold — FastAPI + Next.js AI agent stack with an OEE domain layer (schema, engine, tools, API).

> Base generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).  
> Domain extensions: see [`OEE_DOMAIN.md`](./OEE_DOMAIN.md).
...
| **Database** | PostgreSQL (async via asyncpg) + OEE tables |
| **Auth** | JWT + refresh tokens + API keys |
| **Cache** | Redis |
| **AI Framework** | pydantic_ai (openai) + OEE tools |
...
└── commands/             # Click CLI commands (auto-discovered by `oee_digital_platform cmd …`)
...
The generated project ships a Click CLI exposed as `oee_digital_platform` (after `make install`):

```bash
oee_digital_platform server run --reload          # dev server
oee_digital_platform db upgrade                   # apply migrations
oee_digital_platform db migrate -m "message"      # create new migration
oee_digital_platform user create-admin            # interactive admin creation
oee_digital_platform rag-ingest <path> -c docs    # ingest local files
oee_digital_platform rag-search "query" -c docs   # semantic search
oee_digital_platform rag-collections              # list collections
oee_digital_platform celery worker                # start worker
oee_digital_platform celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `oee_digital_platform --help` for full CLI docs.
...
POSTGRES_DB=oee_digital_platform
...
oee_digital_platform rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
oee_digital_platform rag-search "your query" --collection documents
```
