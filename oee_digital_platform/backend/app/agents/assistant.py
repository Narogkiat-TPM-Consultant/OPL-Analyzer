...
class AssistantAgent:
    ...
    def _register_tools(self, agent: Agent[Deps, str]) -> None:
        ...
        @agent.tool_plain
        async def get_oee_summary_tool(
        ...
        @agent.tool_plain
        async def get_top_downtime_tool(
            plant_code: str | None = None,
            line_code: str | None = None,
            machine_code: str | None = None,
            period_start: str | None = None,
            ...
            """Get Top downtime reasons (Pareto) from plant data.

            Use for Availability loss analysis and "why did we stop" questions.
            """
            return await get_top_downtime(
                plant_code=plant_code,
                line_code=line_code,
                machine_code=machine_code,
                period_start=period_start,
        ...
        @agent.tool_plain
        async def estimate_output_for_target_tool(
            ...
            """Estimate output required to reach a target OEE percent (e.g. 85)."""
            return await estimate_output_for_target(
